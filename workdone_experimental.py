...
class WDCalc():
    ...
    def testgas(self):
        def testspecgas(self,mass):
            """
            Finds the optimal value of gamma and therefore the optimal gas
            """
                self.gammalist.append(self.gamma)
                self.workdone_list.append((self.workdone(0.3)/mass))
            return(self.gammalist,self.workdone_list)

        wb = load_workbook(filename = 'specific_heat_ratio.xlsx')
        sheet_ranges = wb['range names']
        for i in range(50):
         self.gamma=sheet_ranges["A".append[str(i)]].value
         max_gas.append(max(testspecgas(sheet_ranges["B".append[str(i)]].value)[1]))
